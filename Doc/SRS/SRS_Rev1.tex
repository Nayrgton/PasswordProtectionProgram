...
\begin{tabular}{ | p{1cm} | p{4cm} | p{4cm} | p{4cm} | }
	...
	\hline
	FR5 & The new user must be able to add a new entry \textcolor{blue}{to the manager consisting of username, password and type} & To allow the user to store a new username and password \textcolor{blue}{and specify the kind of account it is for} & Create new entry, \textcolor{blue}{by clicking the ‘add’ button, add verify that the application does not give an error and that entry appears} \\
	\hline
	FR6 & The user should be able to generate a random password \textcolor{blue}{ by going to the Generate button} & To allow for a stronger password & Generate multiple PWs and verify randomness \\
	\hline
...
A pop-up notification will display when user is missing a required option when trying to perform a process.\\
Rationale: The User must be notified on why their Application is not performing to follow requirement. \\
Fit Criterion: A notification is sent when a required option is not selected.\\
Priority: HIGH\\
History: Created October 6, 2017
...
Fit Criterion: Options not selected are displayed in the UI \\
Priority: HIGH\\
History: Created October 6, 2017
...
The product shall only store \textcolor{blue}{account types,} usernames and encrypted passwords.\\
...
\caption{2 Factor Authentication \cite{FileCloudDefs:6} }
\end{figure}
